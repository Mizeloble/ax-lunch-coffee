...
시뮬 모델은 달라도 **순위 판정 규칙은 한 벌**이다. 두 러너 다 위치를 이산 샘플링해서(precompute 120 fps, 라이브 60 Hz) 같은 스텝에 골인한 마블이 생기는데, 이걸 배열 인덱스 순으로 세우면 마블 인덱스 = 방 참가 순서라 먼저 들어온 사람이 매번 사진판정을 이긴다. 규칙은 `../marble/finish-order.ts`의 `orderCrossings` 한 곳에만 — 골라인 통과 깊이 내림차순, 정확한 동률은 seed 파생 스트림. 미완주자(스트래글러) 정렬도 같은 함수를 쓴다.

**여기에 인라인 정렬을 다시 쓰지 말 것.** 실제로 한 번 갈렸다 — precompute 쪽만 고치고 라이브 쪽은 안 고쳐서, 코드상 둘을 이어주는 게 아무것도 없었다.

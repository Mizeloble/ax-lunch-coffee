...
## 문제 풀 추가 — 입고 게이트
새 문항은 [questions.ts](questions.ts)의 `NONSENSE_POOL` 끝에 append(`id`는 kebab-case, 유일하게).

**id는 문항 내용을 그대로 담을 것.** 형제 묶음(`EXCLUSIVE_GROUPS`)은 id로만 관리하는데, id가 내용과 어긋나 있으면 어떤 문항이 어느 패밀리인지 눈으로 못 고른다 — 2차 검증에서 미태깅 패밀리가 8개나 남아 있던 구조적 원인이 이것이었다(예: `pun-c-king-fall`이 실제로는 "가장 아름다운 개"였다). 형식 규칙(`question` ~40자, `choices` 각 ~10자, `note` ≤80자)에 더해 **아래 5개 게이트를 전부 통과해야 한다** — 2026-07 큐레이션에서 풀 절반을 덜어낸 원인이 전부 이 게이트 위반이었다:
...
5. **기존 문항과 소재·템플릿·정답이 겹치지 않을 것.** 같은 정답 재사용은 `quiz-pools.test.ts`가 잡는다(숫자형 답 제외). 같은 템플릿("X가 깜짝 놀라면?")의 남발은 리뷰로 거른다. 이미 출하된 닫힌 템플릿 패밀리(회문 3형제, "크게 웃으면?", "길을 잃으면?", 수건·걸레·비누)는 [questions.ts](questions.ts)의 `EXCLUSIVE_GROUPS`로 같은 라운드 동시 출제를 막는다 — 새 문항이 기존 패밀리에 합류하면 거기 추가.

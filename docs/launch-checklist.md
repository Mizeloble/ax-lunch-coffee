...
## G. 서버 용량 검토 (2026-08-15, 토스 유입 대비)

실측 근거 — 로컬(M-series)에서 실제 시뮬레이터를 돌려 측정. 값은 Fly
`shared-cpu-1x` 기준으로 보수적으로 5~10배 derate해서 해석해야 한다.

| 측정 | 결과 |
|---|---|
| 런타임 고정비(node + box2d-wasm) | **~109MB RSS** |
| 방당 추가 메모리(6인, 리플레이 보유) | **~0.7MB** |
| MAX_ROOMS=10 동시 보유 | ~115MB |
| MAX_ROOMS=30 추정 | ~129MB |
| precompute 마블 시뮬 1회 | 6인 9ms / 12인 17ms / **30인 42ms** |
| 10방 동시 시작(최악) | 154ms |
| marble-tilt 라이브(10방×6인, 60Hz) | **코어의 5%**, 브로드캐스트 59KB/s |

**결론: 메모리는 제약이 아니다.** 512MB 중 MAX_ROOMS=50에서도 ~142MB(3.5배 여유).
CPU도 라이브 시뮬 기준 여유가 크다.

**실제 병목은 커넥션 상한**: `fly.toml`의 `hard_limit = 200`. 방당 평균 6명이면
30방 = 180 커넥션으로 상한에 근접한다. 즉 MAX_ROOMS를 올릴 때 **반드시 함께**
올려야 하는 값이다 (`ROOM.MAX_ROOMS` 주석의 "VM 크기 + 커넥션 제한과 같이"가 이것).
...
3. 검수 통과 후 노출 시작 시 `min_machines_running` 0 → **1** 검토:
   ...
   24/7 상시가동은 [월 $3~5 수준](https://fly.io/docs/about/pricing/). 첫 인상이
   중요한 유입 초기에는 켜두고, 트래픽 패턴 확인 후 되돌리는 쪽을 권한다.
4. 그 이상(50방+)이 필요해지면 메모리가 아니라 **커넥션·CPU** 때문에 VM 상향 →
   그 시점엔 `shared-cpu-2x`부터 검토.

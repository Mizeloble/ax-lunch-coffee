...
| 경로 | 비용 | 소요 | 비고 |
|---|---|---|---|
| **(A) 오픈마켓 자체등급분류(IARC)** | 구글 플레이 $25(1회) | 스토어 심사 속도 | **해당 스토어에 게임을 먼저 출시해야 함** → Android 래핑(TWA/WebView) 별도 작업. 플레이 화면 4장(스토어 2 + 앱인토스 2, 무편집) 제출로 동일 게임 증명 |
| **(B) GRAC 직접 신청** | 수수료 발생 | 10~15일 | 안내 문서가 사업자 전제로 서술 — 비사업자 가능 여부 불확실(§7) |
...
**남은 항목 (실기기 확인 필요):**
- [ ] Android 뒤로가기 → 종료 확인 모달 (아직 미보고)
- [ ] `minion.toss.im` 공유 링크 탭 → 미니앱 해당 방 진입 (테스트 배포에서 되는지)
